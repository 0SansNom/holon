...
Connectivity is **migrations-first**. `0000_baseline.sql` is the CREATE
TABLE baseline (idempotent `IF NOT EXISTS`). `0001` is the historical
non-additive follow-up (`auth_header_*` DROP NOT NULL). Schema lives
only in these files — there is no domain `ensure_schema()` / in-module
`DDL` string. Additive changes go in a new `NNNN_*.sql` file.
