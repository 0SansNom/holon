...
import {
  useSyncDataset,
  useDisableSalesforceSource,
  useEnableSalesforceSource,
  useDeleteSalesforceSource,
} from "../../api/hooks";
...
export function SalesforceSourceRow({
  source,
  lastSync,
  onEdit,
}: {
  ...
  const disable = useDisableSalesforceSource();
  const enable = useEnableSalesforceSource();
  const del = useDeleteSalesforceSource();
  const [result, setResult] = useState<{ ok: boolean; message: string } | null>(null);
  const [confirmingDelete, setConfirmingDelete] = useState(false);
  const [creatingType, setCreatingType] = useState(false);

  const busy = sync.isPending || disable.isPending || enable.isPending || del.isPending;
  const soqlPreview = source.soql.length > 80 ? `${source.soql.slice(0, 77)}…` : source.soql;
    ...
    <Card className={`hl-source-row${source.status === "disabled" ? " hl-source-row--disabled" : ""}`}>
      <div className="hl-source-row-header">
        <div>
          ...
          <div className="hl-mono hl-text-muted-sm hl-mt-xs">{soqlPreview}</div>
          <div className="hl-tag-row hl-mt-xs">
            <Tag minimal icon="office">
              Salesforce · {source.api_version}
            </Tag>
        ...
        <div className="hl-source-row-actions">
          ...
          <div className="hl-source-row-buttons">
            ...
            <Button
              ...
              disabled={source.status !== "active" || busy}
