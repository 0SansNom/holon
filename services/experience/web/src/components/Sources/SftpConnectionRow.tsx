...
import { useDeleteSftpConnection } from "../../api/hooks";
import { ApiError } from "../../api/client";
import type { SftpConnection } from "../../api/connectivity";

export function SftpConnectionRow({ connection, onEdit }: { connection: SftpConnection; onEdit: () => void }) {
  const del = useDeleteSftpConnection();
  ...
  return (
    <Card className="hl-source-row">
      ...
      >
        <p>
          Delete <strong>{connection.name}</strong>? Any SFTP source still pointed at it will refuse — repoint or delete
          those first.
        </p>
      </Alert>
    </Card>
