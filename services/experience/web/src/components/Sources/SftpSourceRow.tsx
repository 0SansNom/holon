...
import { useSyncDataset, useDisableSftpSource, useEnableSftpSource, useDeleteSftpSource } from "../../api/hooks";
...
export function SftpSourceRow({
  ...
  const disable = useDisableSftpSource();
  const enable = useEnableSftpSource();
  const del = useDeleteSftpSource();
  const [result, setResult] = useState<{ ok: boolean; message: string } | null>(null);
  const [confirmingDelete, setConfirmingDelete] = useState(false);
  const [creatingType, setCreatingType] = useState(false);

  const busy = sync.isPending || disable.isPending || enable.isPending || del.isPending;
  const pathLabel = source.remote_path ?? `${source.remote_prefix}*`;
    ...
    <Card className={`hl-source-row${source.status === "disabled" ? " hl-source-row--disabled" : ""}`}>
      <div className="hl-source-row-header">
        <div>
          ...
          <div className="hl-mono hl-text-muted-sm hl-mt-xs">sftp://{pathLabel}</div>
          <div className="hl-tag-row hl-mt-xs">
            <Tag minimal icon="folder-shared">
              {source.format}
            </Tag>
