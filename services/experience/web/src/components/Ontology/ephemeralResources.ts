 ...
 * - RelationType unique-name: `ordersViaGenJoin_0118d88c` (underscore + 6–10 hex)
 */

const MS_TIMESTAMP_SUFFIX = /_\d{10,}$/;
const DASH_HEX_SUFFIX = /-[0-9a-f]{6,10}$/i;
const UNDERSCORE_HEX_SUFFIX = /_[0-9a-f]{6,10}$/i;
...
function matchesEphemeralLeaf(name: string): boolean {
  ...
  if (UNDERSCORE_HEX_SUFFIX.test(name)) return true;
