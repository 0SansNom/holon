...
Identity is **migrations-first**. `0000_baseline.sql` is the CREATE TABLE
baseline (idempotent `IF NOT EXISTS`). `0001`–`0002` are the historical
...
- Filename: `NNNN_short_description.sql` — four-digit, zero-padded,
  sequential, lowercase, underscores. `0003_add_widget_kind_column.sql`.
