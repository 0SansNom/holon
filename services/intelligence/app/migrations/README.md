...
Intelligence is **migrations-first**. `0000_baseline.sql` is the CREATE
TABLE baseline (idempotent `IF NOT EXISTS`). Schema lives only in these
files — there is no domain `ensure_schema()` / in-module `DDL` string.
Additive changes go in a new `NNNN_*.sql` file. Data seeds (e.g. starter
gold set) may still run after migrations at boot.
