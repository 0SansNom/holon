"""Browser-session authentication contract for Identity.

Black-box HTTP tests against the running stack (`make up`).  Python's cookie
jar does not send Secure cookies over plain HTTP, so the cookie returned by
`/login` is forwarded explicitly to exercise the server-side cookie path.
"""
