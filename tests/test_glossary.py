"""Business glossary tests (synonyms, house terms). Black-box over HTTP.
...
def test_glossary_is_populated_with_real_terms(jdoe_token: str) -> None:
    status, body = _request("GET", f"{KNOWLEDGE}/glossary", token=jdoe_token)
...
def test_glossary_term_resolves_by_canonical_name(jdoe_token: str) -> None:
    status, body = _request("GET", f"{KNOWLEDGE}/glossary/client", token=jdoe_token)
    assert status == 200, body
    assert "Customer" in body["definition"], body


def test_glossary_term_resolves_by_synonym(jdoe_token: str) -> None:
    status, canonical = _request("GET", f"{KNOWLEDGE}/glossary/client", token=jdoe_token)
    assert status == 200, canonical

    status, via_synonym = _request("GET", f"{KNOWLEDGE}/glossary/customer", token=jdoe_token)
...
def test_unknown_glossary_term_is_404(jdoe_token: str) -> None:
    status, body = _request("GET", f"{KNOWLEDGE}/glossary/doesnotexist", token=jdoe_token)
