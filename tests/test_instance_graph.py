"""End-to-end verification of `GET /objects/{type}/{id}/graph` — the
instance-level N-hop link-analysis endpoint (Vertex-style), a different
granularity entirely from the schema/dataset-level `/lineage` endpoint
already covered by `test_column_lineage.py`. Black-box over HTTP.
...
def full_neighborhood_synced(jdoe_token: str) -> None:
    ...
    endpoint's demo path spans 4 datasets — `POST /sync` has no
    sync-everything mode (`dataset: str = "customers"` is its only
    default), so each one needs its own explicit call.
    """
...
def test_two_hop_neighborhood_covers_orders_tickets_and_reviews(
    ...
    status, graph = _request(
        "GET", f"{KNOWLEDGE}/objects/Customer/{CUSTOMER_WITH_FULL_NEIGHBORHOOD}/graph?hops=2", token=jdoe_token
    )
...
def test_hops_beyond_the_max_are_clamped_not_rejected(jdoe_token: str, full_neighborhood_synced: None) -> None:
    status_capped, graph_capped = _request(
        "GET", f"{KNOWLEDGE}/objects/Customer/{CUSTOMER_WITH_FULL_NEIGHBORHOOD}/graph?hops=3", token=jdoe_token
    )
    status_over, graph_over = _request(
        "GET", f"{KNOWLEDGE}/objects/Customer/{CUSTOMER_WITH_FULL_NEIGHBORHOOD}/graph?hops=99", token=jdoe_token
    )
...
def test_graph_traversal_goes_through_the_same_pdp(alice_token: str, full_neighborhood_synced: None) -> None:
    status, body = _request(
        "GET", f"{KNOWLEDGE}/objects/Customer/{CUSTOMER_WITH_FULL_NEIGHBORHOOD}/graph", token=alice_token
    )
...
def test_graph_for_unknown_instance_is_404(jdoe_token: str, full_neighborhood_synced: None) -> None:
    status, body = _request("GET", f"{KNOWLEDGE}/objects/Customer/999999/graph", token=jdoe_token)
    assert status == 404, body


def test_graph_for_unknown_object_type_is_404(jdoe_token: str) -> None:
    status, body = _request("GET", f"{KNOWLEDGE}/objects/NotAType/1/graph", token=jdoe_token)
    assert status == 404, body
