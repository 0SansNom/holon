...
def test_conformant_implements_publishes_and_is_polymorphically_queryable(msmith_token: str, jdoe_token: str) -> None:
    ...
    list endpoint uses, just reached polymorphically (seeded OT path).
    """
...
def test_self_serve_implementer_is_included_in_interface_objects(
    ...
    not only the six seeded FETCH_FNS — a self-serve OT that implements
    the interface must appear.
    """
    ...
    status, published = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/{object_type_name}/versions/{draft['version']}/publish",
        token=msmith_token,
    )
    assert status == 200, published

    status, objects = _request(
        "GET", f"{KNOWLEDGE}/interfaces/{interface_name}/objects", token=jdoe_token
    )
    assert status == 200, objects
...
def test_tighten_interface_blocked_when_implementer_would_break(msmith_token: str) -> None:
    ...
    must 400 — silent drift is the Foundry-shaped failure mode.
    """
    ...
    status, published = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/Supplier/versions/{draft['version']}/publish",
        token=msmith_token,
    )
    assert status == 200, published

    status, body = _request(
        "PUT", f"{KNOWLEDGE}/interfaces/{interface_name}", token=msmith_token,
...
def test_typed_interface_property_rejects_untyped_implementer(msmith_token: str) -> None:
    ...
    status, published = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/Supplier/versions/{typed_draft['version']}/publish",
        token=msmith_token,
    )
    assert status == 200, published
...
def test_link_constraint_requires_binding_at_publish(msmith_token: str) -> None:
    ...
    status, draft = _request(
        "POST", f"{KNOWLEDGE}/ontology/Order/versions", token=msmith_token,
        body={"implements": [interface_name]},
    )
    assert status == 201, draft
    status, publish_result = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/Order/versions/{draft['version']}/publish",
        token=msmith_token,
    )
    assert status == 400, publish_result
    ...
    status, published = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/Order/versions/{bound['version']}/publish",
        token=msmith_token,
    )
    assert status == 200, published
...
def test_child_interface_inherits_parent_required_property(msmith_token: str) -> None:
    ...
    status, published = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/Supplier/versions/{ok_draft['version']}/publish",
        token=msmith_token,
    )
    assert status == 200, published
...
def test_parent_interfaces_rejects_cycle(msmith_token: str) -> None:
    ...
    status, body = _request(
        "PUT", f"{KNOWLEDGE}/interfaces/{a}", token=msmith_token,
...
def test_parent_objects_includes_child_implementer(
    ...
    status, objects = _request(
        "GET", f"{KNOWLEDGE}/interfaces/{parent}/objects", token=jdoe_token
    )
    assert status == 200, objects
...
def test_tighten_parent_blocked_by_child_implementer(msmith_token: str) -> None:
    ...
    status, body = _request(
        "PUT", f"{KNOWLEDGE}/interfaces/{parent}", token=msmith_token,
        body={"required_properties": ["country", "thisPropertyDoesNotExistOnSupplier"]},
    )
    assert status == 400, body
    detail = body.get("detail", "")
...
def test_search_interface_filter_narrows_to_implementers(msmith_token: str, jdoe_token: str) -> None:
    ...
    Uses Customer (indexed via connectivity sync) rather than Supplier.
    """
    ...
    while time.monotonic() < deadline:
        status, filtered = _request(
            "GET", f"{KNOWLEDGE}/search?q=Acme&interface={parent}&size=50", token=jdoe_token,
        )
    ...
    status, missing = _request(
        "GET", f"{KNOWLEDGE}/search?q=Acme&interface={_unique_name('NoSuchIface')}", token=jdoe_token,
    )
...
def test_delete_interface_blocked_by_implementer_and_succeeds_when_clear(msmith_token: str) -> None:
    ...
    status, body = _request("DELETE", f"{KNOWLEDGE}/interfaces/{iface}", token=msmith_token)
    assert status == 400, body
    ...
    status, cleared = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/Supplier/versions/{clear['version']}/publish",
        token=msmith_token,
    )
    assert status == 200, cleared

    status, deleted = _request("DELETE", f"{KNOWLEDGE}/interfaces/{iface}", token=msmith_token)
    assert status == 200, deleted
    assert deleted["name"] == iface, deleted
    status, missing = _request("GET", f"{KNOWLEDGE}/interfaces/{iface}", token=msmith_token)
    assert status == 404, missing
...
def test_struct_field_binding_satisfies_interface_property(msmith_token: str, jdoe_token: str) -> None:
    ...
    status, publish_missing = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/{object_type_name}/versions/{missing['version']}/publish",
        token=msmith_token,
    )
    ...
    status, published = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/{object_type_name}/versions/{bound['version']}/publish",
        token=msmith_token,
    )
    assert status == 200, published
