"""FK link write / unlink via PUT|DELETE …/objects/{OT}/{id}/links/{name}.

Requires the stack running (`make up`). Mutates only the declarative
`object_instance_edit` overlay — Iceberg source rows are untouched.
"""
...
def test_fk_link_write_and_unlink_round_trip(editor_token: str) -> None:
    status, orders = _request("GET", f"{KNOWLEDGE}/objects/Order", token=editor_token)
    ...
    status, linked = _request(
        "PUT",
        f"{KNOWLEDGE}/objects/Order/{order_id}/links/customer",
        token=editor_token,
        body={"target_id": new_customer},
    )
    assert status == 200, linked
    assert linked["relation"] == "Order.customer", linked
    assert any(int(item["id"]) == int(new_customer) for item in linked["items"]), linked

    status, again = _request(
        "GET", f"{KNOWLEDGE}/objects/Order/{order_id}/links/customer", token=editor_token
    )
    assert status == 200, again
    assert any(int(item["id"]) == int(new_customer) for item in again["items"]), again
    ...
    assert unlinked["items"] == [], unlinked
    ...
    assert any(int(item["id"]) == int(original_customer) for item in restored["items"]), restored


def test_link_write_from_reverse_side_is_rejected(editor_token: str) -> None:
    status, body = _request(
        "PUT",
        f"{KNOWLEDGE}/objects/Customer/1/links/orders",
        token=editor_token,
        body={"target_id": 1},
    )
