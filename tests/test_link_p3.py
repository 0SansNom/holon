...
def test_admin_can_generate_join_dataset(msmith_token: str) -> None:
    ...
    status, body = _request(
        "POST",
        f"{KNOWLEDGE}/catalog/join-datasets",
        token=msmith_token,
    ...
    status, datasets = _request("GET", f"{KNOWLEDGE}/catalog/datasets", token=msmith_token)
...
def test_join_dataset_link_write_and_unlink_via_overlay(msmith_token: str, jdoe_token: str) -> None:
    ...
    status, linked = _request(
        "PUT",
        f"{KNOWLEDGE}/objects/Customer/1/links/{fwd}",
        token=jdoe_token,
        body={"target_id": 1},
    )
    assert status == 200, linked
    assert any(int(item["id"]) == 1 for item in linked["items"]), linked

    status, again = _request(
        "GET", f"{KNOWLEDGE}/objects/Customer/1/links/{fwd}", token=jdoe_token
    )
    assert status == 200, again
    assert any(int(item["id"]) == 1 for item in again["items"]), again

    status, unlinked = _request(
        "DELETE",
        f"{KNOWLEDGE}/objects/Customer/1/links/{fwd}?target_id=1",
        token=jdoe_token,
    )
...
def test_object_backed_link_write_via_overlay(msmith_token: str, jdoe_token: str) -> None:
    ...
    assert status == 200, linked
    assert any(int(item["id"]) == target_order for item in linked["items"]), linked

    status, wb = _request(
        "GET", f"{KNOWLEDGE}/relation-types/{rel_name}/writeback-status", token=jdoe_token
    )
