...
def test_supplier_classification_is_internal(jdoe_token: str, suppliers_synced: dict) -> None:
    # Recomputed asynchronously by the catalog consumer
    # rather than assume convergence, same race every other connector
    # test already handles.
    deadline = time.monotonic() + 30
    object_type: dict = {}
    while time.monotonic() < deadline:
        status, object_type = _request("GET", ontology_url("/objectTypes/Supplier"), token=jdoe_token)
...
def test_suppliers_are_directly_resolvable(jdoe_token: str, suppliers_synced: dict) -> None:
    ...
    suppliers: list = []
    while time.monotonic() < deadline:
        status, suppliers = _request("GET", ontology_url("/objects/Supplier"), token=jdoe_token)
    ...
    status, supplier = _request("GET", ontology_url(f"/objects/Supplier/{first_id}"), token=jdoe_token)
    assert status == 200
    assert supplier["id"] == first_id

    status, body = _request("GET", ontology_url("/objects/Supplier/999999"), token=jdoe_token)
