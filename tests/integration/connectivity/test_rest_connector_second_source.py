...
# Mirrors docker/mongo-init/init.js
...
def test_support_ticket_classification_is_internal_not_confidential(jdoe_token: str, tickets_synced: dict) -> None:
    status, object_type = _request("GET", ontology_url("/objectTypes/SupportTicket"), token=jdoe_token)
    assert status == 200, object_type
    # No confidential column in the mapping
...
def test_relation_traversal_returns_the_right_tickets(jdoe_token: str, tickets_synced: dict) -> None:
    status, body = _request(
        "GET", ontology_url(f"/objects/Customer/{CUSTOMER_WITH_TICKETS}/links/tickets"), token=jdoe_token, unwrap_pages=False
    )
    assert status == 200, body
    tickets = as_items(body)
...
def test_relation_traversal_for_customer_without_tickets_is_empty(jdoe_token: str, tickets_synced: dict) -> None:
    status, body = _request(
        "GET", ontology_url(f"/objects/Customer/{CUSTOMER_WITHOUT_TICKETS}/links/tickets"), token=jdoe_token, unwrap_pages=False
    )
    assert status == 200, body
...
def test_ticket_traversal_goes_through_the_same_pdp(alice_token: str, tickets_synced: dict) -> None:
    status, body = _request(
        "GET", ontology_url(f"/objects/Customer/{CUSTOMER_WITH_TICKETS}/links/tickets"), token=alice_token
    )
...
def test_support_ticket_objects_are_directly_resolvable(jdoe_token: str, tickets_synced: dict) -> None:
    status, tickets = _request("GET", ontology_url("/objects/SupportTicket"), token=jdoe_token)
    assert status == 200
    assert len(tickets) >= EXPECTED_TICKET_COUNT_FOR_CUSTOMER_WITH_TICKETS

    first_id = tickets[0]["id"]
    status, ticket = _request("GET", ontology_url(f"/objects/SupportTicket/{first_id}"), token=jdoe_token)
    assert status == 200
    assert ticket["id"] == first_id

    status, body = _request("GET", ontology_url("/objects/SupportTicket/999999"), token=jdoe_token)
