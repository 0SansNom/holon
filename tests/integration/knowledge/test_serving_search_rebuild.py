"""Tests for Serving Search Rebuild."""
...
def test_serving_store_and_search_reconstruct_from_iceberg_after_being_wiped(jdoe_token: str) -> None:
    ...
    status, before_rows = _request("GET", ontology_url("/objects/Customer"), token=jdoe_token)
    ...
    status, before_search = _request("GET", holon_url("/search?q=Acme"), token=jdoe_token)
    assert status == 200, before_search
    assert before_search["total"] >= 1, before_search

    # 2. Wipe both sinks directly
    ...
    while time.monotonic() < deadline:
        ...
        status, wiped_search = _request("GET", holon_url("/search?q=Acme"), token=jdoe_token)
    ...
    rebuilt_rows: list = []
    while time.monotonic() < deadline:
        status, rebuilt_rows = _request("GET", ontology_url("/objects/Customer"), token=jdoe_token)
    ...
    # Same "Acme" ambiguity as the wipe check above
    # hit specifically.
    deadline = time.monotonic() + 30
    rebuilt_search: dict = {}
    while time.monotonic() < deadline:
        status, rebuilt_search = _request("GET", holon_url("/search?q=Acme"), token=jdoe_token)
