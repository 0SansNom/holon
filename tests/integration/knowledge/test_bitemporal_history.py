...
def test_as_of_read_returns_the_state_that_was_true_at_that_time(jdoe_token: str) -> None:
    ...
    try:
        ...
        status, historical = _request(
            "GET",
            ontology_url(f"/objects/Customer/{CUSTOMER_ID}?as_of={urllib.parse.quote(as_of_before)}"),
            token=jdoe_token,
        )
...
def test_as_of_far_in_the_past_has_no_history(jdoe_token: str) -> None:
    status, body = _request(
        "GET", ontology_url(f"/objects/Customer/{CUSTOMER_ID}?as_of=2000-01-01T00:00:00Z"), token=jdoe_token
    )
