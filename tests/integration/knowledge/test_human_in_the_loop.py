...
def _request_close_account(token: str, customer_id: int, reason: str = "fraud investigation"):
    return _request(
        "POST", ontology_url(f"/objects/Customer/{customer_id}/actions/closeAccount"), token=token, body={"reason": reason}
    )
...
def test_high_risk_action_only_creates_a_pending_approval(jdoe_token: str) -> None:
    ...
    status, customer = _request("GET", ontology_url("/objects/Customer/9"), token=jdoe_token)
    assert status == 200
...
def test_requester_cannot_approve_her_own_request(jdoe_token: str) -> None:
    ...
    status, body = _request("POST", holon_url(f"/approvals/{approval_id}/approve"), token=jdoe_token, body={})
...
def test_admin_approval_applies_the_mutation(jdoe_token: str, msmith_token: str) -> None:
    ...
    status, decision = _request(
        "POST", holon_url(f"/approvals/{approval_id}/approve"), token=msmith_token, body={"note": "confirmed with fraud team"}
    )
    assert status == 200, decision
    assert decision["status"] == "approved"
    assert decision["account_closed"] is True

    status, customer = _request("GET", ontology_url("/objects/Customer/4"), token=jdoe_token)
    assert status == 200
...
def test_admin_rejection_leaves_state_unapplied(jdoe_token: str, msmith_token: str) -> None:
    ...
    status, decision = _request(
        "POST", holon_url(f"/approvals/{approval_id}/reject"), token=msmith_token, body={"note": "insufficient evidence"}
    )
    assert status == 200, decision
    assert decision["status"] == "rejected"

    status, customer = _request("GET", ontology_url("/objects/Customer/6"), token=jdoe_token)
    assert status == 200
...
def test_low_risk_action_still_applies_immediately(jdoe_token: str) -> None:
    status, result = _request(
        "POST",
        ontology_url("/objects/Customer/5/actions/putOnCreditHold"),
        token=jdoe_token,
...
def test_pending_approvals_are_listed(jdoe_token: str, msmith_token: str) -> None:
    ...
    status, pending = _request("GET", holon_url("/approvals?status=pending"), token=msmith_token)
    assert status == 200
