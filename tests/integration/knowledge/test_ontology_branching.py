...
def test_viewer_cannot_create_a_branch(kenji_token: str) -> None:
    status, body = _request(
        "POST", ontology_url("/objectTypes/Supplier/branches"), token=kenji_token,
...
def test_editor_can_create_a_branch(jdoe_token: str) -> None:
    ...
    status, branch = _request(
        "POST", ontology_url("/objectTypes/Supplier/branches"), token=jdoe_token,
    ...
    status, fetched = _request("GET", ontology_url(f"/objectTypes/Supplier/branches/{branch_name}"), token=jdoe_token)
...
def test_creating_a_duplicate_branch_name_is_rejected(jdoe_token: str) -> None:
    ...
    status, _ = _request(
        "POST", ontology_url("/objectTypes/Supplier/branches"), token=jdoe_token, body={"branch_name": branch_name}
    )
    assert status == 201
    status, body = _request(
        "POST", ontology_url("/objectTypes/Supplier/branches"), token=jdoe_token, body={"branch_name": branch_name}
...
def test_editor_cannot_review_their_own_branch(jdoe_token: str) -> None:
    ...
    assert status == 201
    status, body = _request(
        "POST", ontology_url(f"/objectTypes/Supplier/branches/{branch_name}/review"), token=jdoe_token,
...
def test_changes_requested_leaves_branch_open_for_a_follow_up_draft(jdoe_token: str, msmith_token: str) -> None:
    ...
    status, review = _request(
        "POST", ontology_url(f"/objectTypes/Supplier/branches/{branch_name}/review"), token=msmith_token,
    ...
    status, updated = _request(
        "POST", ontology_url(f"/objectTypes/Supplier/branches/{branch_name}/draft"), token=jdoe_token,
    ...
    status, reviews = _request("GET", ontology_url(f"/objectTypes/Supplier/branches/{branch_name}/reviews"), token=jdoe_token)
    assert status == 200
...
def test_approved_review_merges_through_the_real_publish_path(jdoe_token: str, msmith_token: str) -> None:
    ...
    status, live = _request("GET", ontology_url("/objectTypes/Supplier"), token=jdoe_token)
    assert status == 200
    assert live["description"] == marker, live
    assert live["version"] == branch["version"], live

    # Re-reviewing a merged branch must be rejected
...
def test_merge_still_enforces_implements_validation(jdoe_token: str, msmith_token: str) -> None:
    ...
    status, branch_after = _request("GET", ontology_url(f"/objectTypes/Supplier/branches/{branch_name}"), token=jdoe_token)
