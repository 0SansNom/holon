...
def test_replaying_a_frozen_plan_reproduces_the_original_result_despite_a_later_mutation(jdoe_token: str) -> None:
    ...
    try:
        ...
        # *original* result
        # since changed.
        status, replayed = _request("POST", holon_url(f"/execute/{plan_hash_a}/replay"), token=jdoe_token)
...
def test_replay_of_unknown_plan_hash_is_rejected(jdoe_token: str) -> None:
    status, body = _request("POST", holon_url("/execute/doesnotexist/replay"), token=jdoe_token)
