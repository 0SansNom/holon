...
def test_matches_predicates_struct_field_path() -> None:
    mapping = {"id": "id", "address": "address_json"}
    definition = {"all": [{"property": "address.city", "op": "eq", "value": "Paris"}]}
    assert matches_predicates({"address": {"city": "Paris"}}, definition, mapping)
    assert matches_predicates({"address_json": {"city": "Paris"}}, definition, mapping)
    assert not matches_predicates({"address": {"city": "Lyon"}}, definition, mapping)


def test_validate_definition_allows_struct_field_paths() -> None:
    mapping = {"id": "id", "address": "address_json"}
    property_types = {
        "address": {
            "kind": "struct",
    ...
    with pytest.raises(ValueError, match="struct"):
        validate_definition(
            {"all": [{"property": "id.city", "op": "eq", "value": "x"}]},
            mapping,
            property_types,
        )
    with pytest.raises(ValueError, match="unknown struct field"):
        validate_definition(
            {"all": [{"property": "address.zip", "op": "eq", "value": "x"}]},
            mapping,
            property_types,
        )
...
def test_create_list_and_evaluate_object_set(msmith_token: str, jdoe_token: str) -> None:
    ...
    status, listed = _request("GET", ontology_url("/objectSets"), token=jdoe_token)
    assert status == 200, listed
    assert any(s["name"] == name for s in listed), listed

    status, evaluated = _request(
        "GET", ontology_url(f"/objectSets/{name}/objects"), token=jdoe_token
    )
    assert status == 200, evaluated
    assert evaluated["object_set"] == name, evaluated
    assert evaluated["object_type"] == "Order", evaluated
    assert "count" in evaluated and "data" in evaluated, evaluated
    for item in evaluated["data"]:
...
def test_health_check_flags_metadata_gaps(jdoe_token: str) -> None:
    status, findings = _request("GET", ontology_url("/objectTypes/health-check"), token=jdoe_token)
