"""Tests for Relation Type Governance."""
...
def test_seeded_relation_types_are_listed(jdoe_token: str) -> None:
    status, relations = _request("GET", ontology_url("/linkTypes"), token=jdoe_token)
...
def test_editor_cannot_register_a_relation_type(jdoe_token: str) -> None:
    """jdoe is editor-only (never admin, by construction."""
    status, body = _request(
        "POST",
        ontology_url("/linkTypes"),
        token=jdoe_token,
...
def test_nonexistent_target_object_type_is_rejected(msmith_token: str) -> None:
    """The actual enforcement of "extrémités existantes"."""
    status, body = _request(
        "POST",
        ontology_url("/linkTypes"),
        token=msmith_token,
