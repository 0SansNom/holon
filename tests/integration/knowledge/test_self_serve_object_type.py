"""Tests for Self Serve Object Type."""
...
import pytest
from conftest import CONNECTIVITY, IDENTITY, KNOWLEDGE, TENANT_ID, _unique_name, as_items, ontology_url, holon_url
...
def test_creating_an_object_type_requires_admin_governance_tier(jdoe_token: str) -> None:
    ...
    status, body = _request(
        "POST", holon_url("/object-types"), token=jdoe_token,
        body={
...
def test_preview_returns_real_column_names_and_sample_values(jdoe_token: str) -> None:
    ...
    status, preview = _request("GET", holon_url(f"/catalog/datasets/{dataset_name}/preview"), token=jdoe_token)
...
def test_preview_of_a_never_synced_dataset_is_404(jdoe_token: str) -> None:
    status, body = _request(
        "GET", holon_url(f"/catalog/datasets/{_unique_name('never_synced')}/preview"), token=jdoe_token
    )
...
def test_full_self_serve_loop_source_to_browsable_and_searchable_object(jdoe_token: str, msmith_token: str) -> None:
    ...
    status, listing = _request("GET", ontology_url("/objectTypes"), token=jdoe_token)
    ...
    search_result = None
    while time.monotonic() < deadline:
        status, search_result = _request("GET", holon_url("/search?q=milling+machine"), token=jdoe_token)
...
def test_unknown_object_type_is_404_not_500(jdoe_token: str) -> None:
    status, body = _request("GET", ontology_url(f"/objects/{_unique_name('DoesNotExist')}"), token=jdoe_token)
    assert status == 404, body

    status, body = _request("GET", ontology_url(f"/objects/{_unique_name('DoesNotExist')}/1"), token=jdoe_token)
    assert status == 404, body
