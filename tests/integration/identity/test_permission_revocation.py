...
def _kenji_can_read_reviews(kenji_token: str) -> bool:
    status, _ = _request("GET", ontology_url("/objects/ProductReview"), token=kenji_token)
    return status == 200
...
def test_revoke_then_grant_kenji_viewer_access(msmith_token: str, kenji_token: str) -> None:
    ...
    status, body = _request("GET", ontology_url("/objects/ProductReview"), token=kenji_token)
    ...
    # asymmetry the module docstring notes
...
def test_editor_cannot_revoke_or_grant_access(jdoe_token: str) -> None:
    """jdoe is editor-only, never admin."""
    status, body = _request(
