"""Verification of query logging. Black-box over HTTP.
...
def test_search_query_appears_in_the_query_log(jdoe_token: str) -> None:
    ...
    status, search_result = _request("GET", f"{KNOWLEDGE}/search?q={marker}", token=jdoe_token)
    assert status == 200, search_result

    deadline = time.monotonic() + 15
    entry = None
    while time.monotonic() < deadline:
        status, log = _request("GET", f"{KNOWLEDGE}/query-log", token=jdoe_token)
