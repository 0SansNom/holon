...
Black-box: talks to the running stack over HTTP only (`make up` first),
never imports service internals. Requires `pip install -r tests/requirements.txt`.
"""
...
def test_customer_objects_are_resolvable(token: str, synced: dict) -> None:
    status, customers = _request("GET", f"{KNOWLEDGE}/objects/Customer", token=token)
    assert status == 200
...
def test_single_customer_lookup(token: str) -> None:
    status, customer = _request("GET", f"{KNOWLEDGE}/objects/Customer/1", token=token)
    assert status == 200
    assert customer["id"] == 1

    status, _ = _request("GET", f"{KNOWLEDGE}/objects/Customer/9999", token=token)
    assert status == 404
...
def test_lineage_links_dataset_version_to_object_type(token: str, synced: dict) -> None:
    ...
    status, edges = _request("GET", f"{KNOWLEDGE}/lineage/{synced['dataset_version_urn']}", token=token)
    assert status == 200
...
def test_dashboard_only_talks_to_the_knowledge_api() -> None:
    """The shipped SPA never leaks storage internals, and its JS calls real
    service APIs (Identity/Knowledge/Experience over HTTP) rather than any
    storage layer directly — the SPA now calls those services straight from
    the browser (CORS), not through an Experience-side proxy, so this checks
    the built JS bundle rather than a server-rendered page.
    """
