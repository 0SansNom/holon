"""End-to-end verification of the Automation Platform — the Workflow Engine that owns saga
orchestration for `Customer.closeAccount`.
Proves Automation's *own* persisted execution record (`workflow_execution`,
exposed via `GET /workflows/{approval_id}`), not just the downstream
effects `test_saga_compensation.py` already checks. Black-box over HTTP,
...
import pytest
from conftest import AUTOMATION, IDENTITY, KNOWLEDGE, _request
