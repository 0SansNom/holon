"""agentApp —  (AIP-equivalent
no-code tooling). A non-engineer configures a bounded agent through
Experience's Application Builder — a tool allowlist, a system-prompt
template, and budget defaults — without touching what Agent Runtime
itself does. Proves: an `agentApp` surface's declared tools are checked
against Intelligence's real, live tool catalog (`GET /tools`) at
definition time, same as `objectApp`/`form` surfaces already check
against Knowledge's ontology; `systemPrompt`/`budget` get real
structural validation; `POST .../agent-sessions` compiles the surface
into a real Intelligence session (verified by reading the session back
directly, authenticated as the shared `ingest-bot` identity every
agentApp session runs under) with the configured tools/prompt/budget
actually applied — never the full unrestricted tool set or the default
prompt; and the session-ownership boundary Experience adds on top holds:
only the principal who launched a given session may drive its turns,
even though every agentApp session shares the same underlying agent
identity. Turn/replay execution itself (a real LLM call) is
`test_agent_runtime.py`'s job, not this file's — no real LLM calls here.
"""
...
def ingest_bot_token() -> str:
    """The shared identity every agentApp session runs under — seeded
    with its own deterministic dev credential, same as every other
    principal in `identity/app/seed.py`. Used here only to read a
    session back directly and confirm what Experience actually compiled
    into it, not to drive a real conversation.
    """
...
def weather_lookup_registered(jdoe_token: str) -> None:
    ...
    entry in Intelligence's `GET /tools` for this test's allowlist check.

    Registration itself is a plain HTTP call, not a real LLM round trip —
    `test_agent_tool_plugin.py` is the file that actually *invokes* this
    plugin through a live Claude call, which is why that whole file
    carries `pytestmark = pytest.mark.llm` and is excluded from the
    default `pytest -k "not llm"` run. This test was previously relying
    on that excluded file having already run first to leave the plugin
    registered — true only by accident of leftover DB state, false on a
    clean stack or under `-k "not llm"` (pytest collects `test_agent_app.py`
    before `test_agent_tool_plugin.py` alphabetically regardless).
    Registering it directly here removes that undeclared cross-file,
    cross-marker dependency.
    """
...
def test_a_real_action_tool_and_a_real_agent_tool_plugin_are_both_accepted(
    ...
    (a synthetic agent-tool-plugin, no ontology backing) are both real
    entries in Intelligence's `GET /tools` — proves the allowlist check
    covers both sources `_list_tools` itself compiles, not just Actions.
    """
...
def test_session_compiles_the_declared_tools_prompt_and_budget(
    ...
    # max_tokens wasn't declared — must still carry the runtime default,
    ...
    # identity itself — confirms what actually landed in Postgres, not
...
def test_only_the_launching_principal_can_drive_their_agent_app_session(
    ...
    # msmith never launched this session — rejected before ever reaching
