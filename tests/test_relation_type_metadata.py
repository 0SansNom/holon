...
def test_admin_can_create_with_side_metadata_and_delete(msmith_token: str, jdoe_token: str) -> None:
    ...
    status, updated = _request(
        "PUT",
        f"{KNOWLEDGE}/relation-types/{name}",
        token=msmith_token,
        body={
    ...
    status, perms = _request("GET", f"{KNOWLEDGE}/relation-types/{name}/permissions", token=msmith_token)
    assert status == 200, perms
    assert perms["permissions"]["approve"] is True, perms

    status, deleted = _request("DELETE", f"{KNOWLEDGE}/relation-types/{name}", token=msmith_token)
    assert status == 200, deleted
    assert deleted.get("deleted") is True, deleted

    status, missing = _request("GET", f"{KNOWLEDGE}/relation-types/{name}", token=jdoe_token)
    assert status == 404, missing


def test_cannot_delete_active_relation_type(msmith_token: str) -> None:
    status, body = _request("DELETE", f"{KNOWLEDGE}/relation-types/Order.customer", token=msmith_token)
    assert status == 400, body
    assert "active" in body["detail"], body


def test_editor_cannot_delete_relation_type(jdoe_token: str) -> None:
    status, body = _request("DELETE", f"{KNOWLEDGE}/relation-types/Order.customer", token=jdoe_token)
