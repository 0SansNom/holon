"""Model Integration —
deliberately bounded, see `model_registry.py`'s own module docstring:
registers and serves an *already-trained* scikit-learn artifact, no
training infrastructure, experiment tracking, or GPU orchestration
attempted. Proves: an artifact that doesn't deserialize as a real model
is rejected at registration time; a registered model produces real,
deterministic predictions (not a stub); missing features and unknown
models are rejected/404 appropriately; disable/enable flips serving
status; the model is genuinely callable from a  Function as a
derived-property computation (`customer_value_model_function.py`), with
R8.7 masking correctly skipping it when its input was masked — the same
guarantee `test_functions.py` already proves for the rule-based
`lifetime_tier` Function. Also verifies fault tolerance for derived properties:
when a Function's external I/O fails (e.g. a disabled model), the read
succeeds with only that property omitted. No real LLM calls.

`_MODEL_ARTIFACT_B64` is a real, tiny `DecisionTreeClassifier` (`lifetimeValue`
-> value tier 0/1/2), trained and `joblib`-dumped once via
`docker compose exec intelligence python3 -c "..."` using the exact
scikit-learn version pinned in `services/intelligence/requirements.txt` —
deliberately not trained here (this file has no `fit()` call either,
same scope boundary as the module it's testing) and deliberately not
trained by whatever scikit-learn version might or might not be on the
host running pytest, sidestepping any version-skew risk between a local
install and the server's own pinned one. Regenerate with the same
snippet (see `services/intelligence/app/model_registry.py`'s module
docstring for the reasoning) if the artifact ever needs to change.
"""
...
def test_register_and_predict_real_values(jdoe_token: str) -> None:
    ...
    # a stub — a low lifetimeValue and a high one land in different tiers.
...
def test_model_backed_function_is_a_real_derived_property_and_respects_masking(
    ...
    `customer-value-classifier` model name — register under exactly that
    name, then prove it through the *real* derived-property path
    (`_apply_derived_properties`/`_mask_and_derive`), the same mechanism
    `test_functions.py` already proves for the rule-based `lifetime_tier`
    Function, not a direct call to Intelligence.
    """
    ...
    # Customer 1 (Acme Robotics) seeds at lifetimeValue 184500 — a
    ...
    # derived property — whose required input is missing — must be
...
def test_a_failing_model_backed_derived_property_degrades_gracefully_not_a_500(
    ...
    underlying model is disabled or unreachable) is caught so the overall object read
    succeeds with the property omitted.
    """
    _register_model(msmith_token, "customer-value-classifier")
    status, draft = _request(
        "POST", f"{KNOWLEDGE}/ontology/Customer/versions", token=msmith_token,
