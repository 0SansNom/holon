"""Connector Plugin SDK tests (`libs/holon_common/plugin.py`,
`services/connectivity/app/plugin_registry.py`). Proves manifest versioning,
activation/deactivation, dataset ownership guards, and execution.

Two throwaway plugin modules are written directly into
`services/connectivity/app/plugins/` (bind-mounted into the running
container, so a fresh module is importable immediately) purely to prove
the conflict guard against a *second* plugin — self-cleaning, removed in
a `finally` block, same self-cleaning idiom this suite already uses
elsewhere (e.g. `test_permission_revocation.py`).
"""
...
PLUGINS_DIR = Path(__file__).resolve().parent.parent / "services" / "connectivity" / "app" / "plugins"
