"""ObjectType metadata (PK / title / lifecycle / visibility) + title_of.

Pure helpers are exercised by importing the ontology submodule with
minimal stubs; HTTP cases require the stack (`make up`).
"""
...
def test_create_object_type_persists_primary_and_title_keys(msmith_token: str) -> None:
    ...
    status, fetched = _request("GET", f"{KNOWLEDGE}/ontology/{type_name}", token=msmith_token)
...
def test_publish_rejects_primary_key_change_when_active(msmith_token: str) -> None:
    status, customers = _request("GET", f"{KNOWLEDGE}/ontology/Customer", token=msmith_token)
    ...
    status, refused = _request(
        "POST",
        f"{KNOWLEDGE}/ontology/Customer/versions/{bad_draft['version']}/publish",
        token=msmith_token,
    )
