"""Tests for the **execution adapter** and **export format**
plugin types. Proves manifest, activation/deactivation, adapter isolation, and masked export serialization.
"""
...
def test_register_execution_adapter_and_route_through_it(jdoe_token: str) -> None:
    status, registration = _request(
        "POST",
        f"{KNOWLEDGE}/execution-adapter-plugins",
        token=jdoe_token,
...
def test_disabling_and_enabling_execution_adapter_flips_registry_status(jdoe_token: str) -> None:
    ...
    try:
        status, disabled = _request(
            "POST", f"{KNOWLEDGE}/execution-adapter-plugins/serving-store-adapter/disable", token=jdoe_token
        )
        assert status == 200 and disabled["status"] == "disabled", disabled
    finally:
        status, enabled = _request(
            "POST", f"{KNOWLEDGE}/execution-adapter-plugins/serving-store-adapter/enable", token=jdoe_token
        )
...
def test_register_export_format_and_export_via_it(jdoe_token: str) -> None:
    status, registration = _request(
        "POST",
        f"{KNOWLEDGE}/export-format-plugins",
        token=jdoe_token,
    ...
    status, body = _request("GET", f"{KNOWLEDGE}/objects/Customer/export?format=csv", token=jdoe_token, raw=True)
    assert status == 200, body
    text = body.decode()
    assert text.startswith("id,"), text
    assert "email" in text.splitlines()[0], text

    status, unknown = _request("GET", f"{KNOWLEDGE}/objects/Customer/export?format=xml", token=jdoe_token)
    assert status == 400, unknown


def test_export_inherits_r87_masking(kenji_token: str) -> None:
    status, body = _request("GET", f"{KNOWLEDGE}/objects/Customer/export?format=csv", token=kenji_token, raw=True)
    assert status == 200, body
