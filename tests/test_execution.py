"""End-to-end verification of the Execution Plan / Adapter abstraction —
one operator (equality filter), one adapter (DuckDB),
plan-hash caching pinned to the exact DatasetVersion read.
...
def test_a_new_sync_invalidates_the_cache_via_a_new_plan_hash(jdoe_token: str, orders_synced: dict) -> None:
    ...
    # sync event. Poll until then — same convergence pattern as
    # test_walking_skeleton's test_dataset_is_catalogued — otherwise the
